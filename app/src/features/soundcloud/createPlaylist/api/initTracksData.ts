...
export function InitFavoriteTracksData(): Promise<void> {
    ...
    favoriteInitPromise = fetchWithRetry('/api/soundcloud/tracks/init/favorites', 'POST', {
        timeout: 0
    }).finally(() => {
...
export function InitFollowedArtistsTracksData(): Promise<void> {
    ...
    followedArtistsInitPromise = fetchWithRetry('/api/soundcloud/tracks/init/followed-artists', 'POST', {
        timeout: 0
    }).finally(() => {
